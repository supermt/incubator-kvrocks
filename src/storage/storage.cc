...
Status Storage::OpenAsSec() {
  ...
  LOG(INFO) << "Trying to open the secondary db at path: " << config_->sec_dir;
  std::cout << "Trying to open the secondary db at path: " << config_->sec_dir << std::endl;
  s = rocksdb::DB::OpenAsSecondary(options, config_->db_dir, config_->sec_dir, column_families, &cf_handles_, &db_);
  if (!s.ok() || db_) {
    s = db_->TryCatchUpWithPrimary();
  } else {
    return {Status::DBOpenErr, s.ToString()};
  }
...
Status Storage::ReOpenDB(bool read_only) {
  CloseDB();
  return Open(read_only);
}
